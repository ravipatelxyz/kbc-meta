...
def main(args):
    ...
    reg_weight_graph = torch.tensor(regweight_init, requires_grad=True, device=device)
    ...
    for outer_step in range(outer_steps):
        ...
        if not is_quiet:
            logger.info(f"outer dev loss: {loss_outer_dev.item():.7f}, reg param: {reg_weight_graph.item():.7f}, reg param gradient: {gradients_outer[-1]}")
