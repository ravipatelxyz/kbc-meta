...
def _maybe_mask(tensor: _torch.Tensor, mask: _torch.Tensor) -> None:
    # print("XXX", tensor.requires_grad)
        ...
        for group_idx, (group, grads) in enumerate(zipped):
            for p_idx, (p, g) in enumerate(zip(group['params'], grads)):
                ...
                if g.is_sparse:
                    ...
                    # print(state['sum'].cpu().detach().numpy())
                    # print(mask.cpu().detach().numpy())

                    sum_sqrt = state['sum'].sqrt()
                    # _maybe_mask(sum_sqrt, mask)

                    std = _add(sum_sqrt, group['eps'] if 'eps' in group else 1e-10)
                    # _maybe_mask(std, mask)
