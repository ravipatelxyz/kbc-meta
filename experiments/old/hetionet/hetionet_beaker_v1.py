...
def to_cmd(c, _path=None):
    command = f'PYTHONPATH=. python3 ./bin/kbc-cli.py ' \
        f'--train data/hetionet/train.tsv ' \
        f'--dev data/hetionet/valid.tsv ' \
        f'--test data/hetionet/test.tsv ' \
...
def to_logfile(c, path):
    outfile = "{}/hetionet_beaker_v1.{}.log".format(path, summary(c).replace("/", "_"))
    return outfile
...
def main(argv):
    ...
    path = 'logs/hetionet/hetionet_beaker_v1'
    is_rc = False
