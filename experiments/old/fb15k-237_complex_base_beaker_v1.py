...
def main(argv):
    # Using F2 regularisation
    hyp_space_1 = dict(
        m=['complex'],
        k=[100, 200, 500, 1000, 2000, 4000], # embedding size
        b=[50, 100, 500],  # training batch size
        e=[100],  # number of epochs
        f2=[1e-3, 5e-3, 1e-2, 5e-2, 1e-1, 5e-1],  # F2 reg param
        n3=[0],  # N3 reg param
        lr=[0.1],
        # i=['standard', 'reciprocal'],
        i=['standard'],
        V=[1],  # How frequently to evaluate on val set (1 = every epoch)
        o=['adagrad'],
        c=['so', 'spo']  # Corruption
    )

    # Using N3 regularisation
    hyp_space_2 = dict(
        m=['complex'],
        k=[100, 200, 500, 1000, 2000, 4000],  # embedding size
        b=[50, 100, 500],  # training batch size
        e=[100],  # number of epochs
        f2=[0],  # F2 reg param
        n3=[1e-3, 5e-3, 1e-2, 5e-2, 1e-1, 5e-1], # N3 reg param
        lr=[0.1],
        # i=['standard', 'reciprocal'],
        i=['standard'],
        V=[1],  # How frequently to evaluate on val set (1 = every epoch)
