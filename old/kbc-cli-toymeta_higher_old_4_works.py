...
def main():
    ...
    for outer_step in range(outer_steps):
        ...
        for epoch_no in range(1, nb_epochs + 1):
            ...
            for batch_no, (batch_start, batch_end) in enumerate(batcher.batches, 1):
                ...
                po_scores = model.forward(xp_batch_emb, None, xo_batch_emb, entity_embeddings=parameters_lst_lh[0], predicate_embeddings=parameters_lst_lh[1])
                ...
                sp_scores = model.forward(xp_batch_emb, xs_batch_emb, None, entity_embeddings=parameters_lst_lh[0], predicate_embeddings=parameters_lst_lh[1])
        ...
        if meta_loss_type == "||B-C||":
            meta_loss = torch.norm(parameters_lst_lh[0][data.entity_to_idx['B']] - parameters_lst_lh[0][data.entity_to_idx['C']])
    ...
    print(f"FINAL entity embeddings: {parameters_lst_lh[0]}")
    print(f"FINAL predicate embedding: {parameters_lst_lh[1]}")
    print(f"FINAL reg param value, p: {reg_param.weight}") # todo make sure this returns the updated and not initialised value
    print(f"FINAL reg term, ||B-C||: {p_target_regularizer(parameters_lst_lh[0][1], reg_param.weight)}")

    print(f"\nstarting inner loss: {mean_losses[0]}")
    print(f"final inner loss: {mean_losses[-1]}")
