...
def main(args):
    ...
    reg_weight_graph = torch.tensor(regweight_init, requires_grad=False).to(device)
    ...
    diffopt = higher.get_diff_optim(optimizer, [e_graph, p_graph], track_higher_grads=False)
    ...
    for epoch_no in range(1, nb_epochs + 1):
        ...
        for batch_no, (batch_start, batch_end) in enumerate(batcher.batches, 1):
            ...
            if regularizer == "F2":
                batch_loss_train += reg_weight_graph * F2_reg(factors)

            if regularizer == "N3":
                batch_loss_train += reg_weight_graph * N3_reg(factors)
        ...
        with torch.no_grad():
            ...
            if loss_inner_dev < best_loss_inner_dev:
                best_loss_inner_dev = loss_inner_dev
